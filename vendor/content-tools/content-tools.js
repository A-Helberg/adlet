...
(function() {
  ...
  ContentEdit.Text = (function(_super) {
    ...
    Text.prototype.getLineRange = function(contents) {
      var characters, i, range, _i, _ref;
      characters = contents.characters;
      range = ContentSelect.Range.query(this._domElement);
      range._to = contents.length();
      for (i = _i = _ref = characters.length - 1; _i >= 0; i = _i += -1) {
        if (characters[i]._tags.length > 0 && characters[i]._tags[0]._name === "br") {
          range._from = i + 1;
          return range;
        }
      }
    ...
    Text.prototype.contentsToString = function(contents) {
      var char, string, _i, _len, _ref;
      string = "";
      _ref = contents.characters;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        char = _ref[_i];
        string = string.concat(char._c);
      }
      return string;
    };
    ...
    Text.prototype._onKeyUp = function(ev) {
      ...
      lineRange = this.getLineRange(tip);
      lineTip = tip.substring(lineRange.get()[0], lineRange.get()[1]);
      tipWithoutLine = this.content.substring(0, selection.get()[0]- lineTip.length());
      lineString = this.contentsToString(lineTip);
      tail = this.content.substring(selection.get()[1]);
      switch (true) {
        case h1.test(lineString):
          console.log("It's a header john!!!");
          this.childElement(this, tip, tail, 'h1');
        break;
        case h2.test(lineString):
          console.log("It's a header2 john!!!");
          this.childElement(this, tip, tail, 'h2');
        break;
        case h3.test(lineString):
          console.log("It's a header3 john!!!");
          this.childElement(this, tip, tail, 'h3');
        break;
        case h4.test(lineString):
          console.log("It's a header4 john!!!");
          this.childElement(this, tip, tail, 'h4');
        break;
        case h5.test(lineString):
          console.log("It's a header5 john!!!");
          this.childElement(this, tip, tail, 'h5');
        break;
        case h6.test(lineString):
          console.log("It's a header6 john!!!");
          this.childElement(this, tip, tail, 'h6');
        break;
        case ol.test(lineString):
          console.log("It's a ordered list john!!!");
          this.childElement(this, tip, tail, 'ol');
        break;
        ...
        case pre.test(lineString):
          ...
          this.childElement(this, tip, tail, 'pre');
        break;
        case quote.test(lineString):
          console.log("It's a quote john!!!");
          this.childElement(this, tip, tail, 'quote');
        break;
      }
    ...
    Text.prototype.childElement = function(currentElement, tip, tail, newElementType) {
      element = new currentElement.constructor(newElementType, {}, tail.trim());
      currentElement.parent().attach(element, currentElement.parent().children.indexOf(currentElement) + 1);
      if (tip.length()) {
        element.focus();
        selection = new ContentSelect.Range(0, 0);
        selection.select(element.domElement());
        ...
        selection = new ContentSelect.Range(0, tip.length());
        selection.select(currentElement._domElement);
      }
      lineRange = this.getLineRange(tip);
      lineTip = tip.substring(lineRange.get()[0], lineRange.get()[1]);
